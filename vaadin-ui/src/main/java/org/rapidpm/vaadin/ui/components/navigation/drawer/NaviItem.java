...
import static org.rapidpm.vaadin.ui.util.UIUtils.createButton;
...
public class NaviItem extends Div {
  ...
  public NaviItem(VaadinIcon icon, String text, Class<? extends Component> navigationTarget) {
    ...
    link
        .getElement()
        .insertChild( 0, new Icon( icon ).getElement() );
  }
  ...
  public NaviItem(Image image, String text, Class<? extends Component> navigationTarget) {
    ...
    link
        .getElement()
        .insertChild( 0, image.getElement() );
  }
  ...
  public NaviItem(String svg, String text, Class<? extends Component> navigationTarget) {
    ...
    try {
      ...
      link
          .getElement()
          .insertChild( 0, createSVGContainer( content ) );
    } catch (Exception e) {
  ...
  public NaviItem(String text, Class<? extends Component> navigationTarget) {
    ...
    expandCollapse = createButton( up, LUMO_SMALL, LUMO_TERTIARY );
  ...
  private void setSubItemsVisible(boolean visible) {
    if (level == 0) {
      expandCollapse.setIcon( new Icon( visible
                                        ? up
                                        : down ) );
    }
  ...
  protected static String readFile(String path) throws IOException {
    try (InputStream resourceAsStream = VaadinService
        .getCurrent()
  ...
  public boolean isHighlighted(AfterNavigationEvent e) {
    return link instanceof RouterLink && ((RouterLink) link)
        ...
        .shouldHighlight( (RouterLink) link, e );
  }
